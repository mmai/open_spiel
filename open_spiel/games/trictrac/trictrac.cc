...
const GameType kGameType{/*short_name=*/"trictrac",
                         /*long_name=*/"Trictrac",
...
Player TrictracState::CurrentPlayer() const {
  if (engine_->is_game_ended())
    return kTerminalPlayerId;
  if (engine_->needs_roll())
...
std::vector<Action> TrictracState::LegalActions() const {
  if (IsChanceNode()) {
    ...
    for (int i = 0; i < kNumChanceOutcomes; ++i)
      outcomes[i] = i;
    return outcomes;
  }
  try {
...
void TrictracState::DoApplyAction(Action action) {
  try {
    ...
    SpielFatalError(
        absl::StrCat("TrictracState::DoApplyAction failed: ", e.what()));
  }
}
...
std::string TrictracState::ActionToString(Player player, Action action) const {
  ...
  return std::string(engine_->action_to_string(static_cast<uint64_t>(player),
