...
void TrictracState::DoApplyAction(Action action_id) {
  if (needs_roll_) {
    ...
    // For simplicity, we'll assume a fixed roll for now.
    // A full implementation would use the action_id to set the dice.
    current_dice_ = Dice(1, 2); // Example roll
    needs_roll_ = false;
    return;
  }
  ...
  Color color = (current_player_ == Player::kPlayer0) ? Color::kWhite : Color::kBlack;
...
std::vector<Action> TrictracState::LegalActions() const {
  ...
  std::vector<Action> actions;
  std::vector<std::pair<CheckerMove, CheckerMove>> legal_moves = GenerateLegalMoveSequences();
...
std::vector<std::pair<CheckerMove, CheckerMove>> TrictracState::GenerateLegalMoveSequences() const {
...
Action TrictracState::MoveSequenceToAction(const std::pair<CheckerMove, CheckerMove>& moves) const {
    // Encode a move sequence into a single integer.
    // from/to are in [0, 24]. So we use base 25.
    return moves.first.from * 25 * 25 * 25 +
           moves.first.to * 25 * 25 +
           moves.second.from * 25 +
           moves.second.to;
}

std::pair<CheckerMove, CheckerMove> TrictracState::ActionToMoveSequence(Action action_id) const {
...
std::string TrictracState::ActionToString(Player player, Action action_id) const {
...
TrictracGame::TrictracGame(const GameParameters& params) : Game(kGameType, params) {}
